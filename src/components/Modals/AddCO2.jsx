...
class AddModalidade extends React.Component {
    ...
    handleSave() {
        if (this.state.nome===""){
            ...
            const data = {modalidade: this.state.nome, logo: this.state.logo, componentes: this.state.componentes, tipo: this.state.tipo, ano: this.state.ano};
      ...
      <>
        <Modal isOpen={this.state.modal} toggle={this.props.handleClose}>
            <ModalHeader className="justify-content-center" toggle={this.props.handleClose}>
                Adicionar política de CO2
            </ModalHeader>
            <ModalBody>
                <form>
                    <FormGroup>
                        <Label for="nome">Nome</Label>
                        <Input type="text" name="nome" id="nome" placeholder="Nome da política" value={this.state.nome} onChange={this.handleNome} />
                    </FormGroup>
                    <div hidden={!this.state.emptyNome} className="alert-danger mt-2 p-2" style={{borderRadius: 30}}>
                        Field can't be empty!
                    </div>
                    <FormGroup>
                        <Label for="logo">Valores de CO2</Label>
                        <Input type="number" name="logo" id="logo" placeholder="Valor de CO2" value={this.state.logo} onChange={this.handleLogo} />
                    </FormGroup>
                    ...
                    <FormGroup>
                        ...
                        <Input type="text" name="ano" id="ano" placeholder="Ação a tomar" value={this.state.ano} onChange={this.handleAno} />
                    </FormGroup>
